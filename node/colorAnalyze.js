...
const prompt = {
    inject: (hex, rgb) => {
        return `ユーザが指定した色 HEXコード${hex} および RGB値 ${rgb} から想起される事物について、下記のテキストに沿って述べてください。
        条件：
        //生成する文章について
        ・回答は日本語で行なってください。
...
app.get('/colorAnalyze', async(req, res) => {
    ...
    console.log(model, rgb, hex);

    let answer;

    if(model == 'gemini'){
        answer = await geminiRes(hex, rgb);

    } else if (model == 'claude'){
        answer = await claudeRes(hex, rgb);
    }
...
async function claudeRes(hex, rgb){

    
    const response = await ai_claude.messages.parse({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 2048,
        messages: [
            {
                role: 'user',
                content: prompt.inject(hex, rgb)
            }
        ],
        output_config: {
            format: zodOutputFormat(analyzeSchema),
        }
    });
...
async function geminiRes(hex, rgb){
    
    const response = await ai.models.generateContent({
        model: "gemini-3.1-flash-lite",
        contents: prompt.inject(hex, rgb),
        config: {
